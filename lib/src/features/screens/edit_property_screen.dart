...
class _EditPropertyScreenState extends State<EditPropertyScreen> {
  ...
  String? dropdownvalueOfChooseService = "Plumbing";
  String? dropdownvalueOfroom = "Semi furnished";
                    ...
                    children: [
                      ...
                      MaterialButton(
                        onPressed: () {
                          ...
                          Helper.checkInternet(editproperyApi());
    ...
    try {
      ...
      if (res.statusCode == 200) {
        try {
          ...
          if (model.status == "true") {
            // restaurent.addAll(model.propertyList!);
            // // restaurent.addAll(_getMyPropertyList.propertyList);
            //
            //
            //
            // _restaurent.clear();
            // for(int i=0; i< restaurent.length; i++){
            //   _restaurent.add(restaurent[i].category.toString());
            //   newMapOfMonths[restaurent[i].category] = restaurent[i].propertyId;
            //   print(newMapOfMonths);
            //
            // }
            setState(() {
  ...
  Future<void>editproperyApi() async {
    print("<=============editproperyApi =============>");
    setProgress(true);
    final prefs = await SharedPreferences.getInstance();
    var user_id=   await prefs.getString('user_id');
    ...
    Map data = {
      'user_id': user_id.toString(),
      'property_id':widget.service_id.toString(),
      'title':titlecontroller.text.toString(),
      'description':descriptioncontroller.text.toString(),
      'address':addresscontroller.text.toString(),
      'mobile':numbercontroller.text.toString(),
    ...
    print("Request =============>" + data.toString());
    try {
      var res = await http.post(Uri.parse(Api.editProperty), body: data);
      print("Response ============>" + res.body);
  ...
  void dataGet(){
    ...
    setState(() {
      ...
      titlecontroller.text = widget.service_name.toString();
